...
import { getPaperMonthlyTotals } from "../services/paperService";
...
const PaperPage = () => {
  ...
  const lineColors = ["#f39c12", "#e74c3c", "#9b59b6", "#3498db", "#2ecc71", "#1abc9c"];
  ...
  // Fetch paper data when selected building or date range changes
  useEffect(() => {
    if (selectedBuildingId === "all") {
      ...
      fetchPaperData(selectedBuildingId);
    }
  ...
  // Function to fetch paper data for a specific building and date range
  const fetchPaperData = async (buildingId) => {
    try {
      setIsLoading(true);
      setError(null);
      
      console.log("Fetching paper data with params:", {
        buildingId,
  ...
  // Function to fetch paper data for all buildings
  const fetchAllBuildingsData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      
      console.log("Fetching all buildings paper data with date range:", {
        startDate: startDate.toISOString(),
        endDate: endDate.toISOString()
      });
      
      // Call getPaperMonthlyTotals, NOT filterPapers
      const result = await getPaperMonthlyTotals(startDate, endDate);
      console.log("All buildings paper data received:", result);
      
      // Transform the API data into the chart format
      const transformedData = processAllBuildingsData(result);
      setMonthlyData(transformedData);
    } catch (error) {
      setError("Failed to fetch all buildings paper data. Please try again later.");
      console.error("Error fetching all buildings paper data:", error);
    } finally {
      setIsLoading(false);
    }
  };
        ...
        {!isLoading && !error && (
          <>
            ...
            <div style={{ 
              ...
              <h3>
                {selectedBuildingId === "all" 
                  ? "Total Paper Usage Across All Buildings" 
                  : "Paper Consumption Graphs"}
              </h3>
            ...
            <div style={{ 
              ...
              <h3>Paper Consumption Comparison By Buildings</h3>
